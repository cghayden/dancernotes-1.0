...
$(document).ready(function() {
    ...
    let customHtml = `
		<div class="col-12 col-md-6 dance-container" id="${id}">
			<div class="dance-header row" data-toggle="collapse" href="#${id}-body">
				<div class="col-1 text-left align-self-center time">
					<h5>${num}</h5>
				</div>
			<div class="col-9 name">
				<h4>${title}</h4>
				<p class="highlight-cool dance-song">${song}</p>
			</div>
			<div class="col-2 align-self-center time">
				<p>${day}</p>
				<h5>${time}</h5>
			</div>
		</div>
  ...
  function renderEditForm(danceId) {
    ...
    for (var i = 0; i < customDances.length; i++) {
      /// prefill custom inputs with existing values.
      if (customDances[i].id === danceId) {
    ...
    for (var i = 0; i < studioDances.length; i++) {
      if (studioDances[i].id === danceId) {
        ...
        $("input[value=" + danceId + "]")
          .parent()
          .parent()
          .hide(); // hide from Setup
        $("#" + danceId).hide(); // hide main Div
        var restoreListItemHtml =
          '<li class="' +
          danceId +
          ' list-group-item restore">' +
          studioDances[i].name +
          "  -  " +
          studioDances[i].song +
          '<button class="btn btn-danger btn-sm">Restore</button></li>';
        $("#edit-delete-list").append(restoreListItemHtml);
      }
    }
  }
